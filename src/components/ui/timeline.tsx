  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
        <h2 className="text-lg md:text-4xl mb-4 text-foreground max-w-4xl font-serif">
          솔로 사우나 이야기
        </h2>
        <p className="text-muted-foreground text-sm md:text-base max-w-sm">
          우리가 함께 만들어가는 공간에 대한 이야기입니다.
        </p>
      </div>
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl font-bold text-primary font-serif">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-bold text-primary font-serif">
                {item.title}
              </h3>
              {item.content}
            </div>
